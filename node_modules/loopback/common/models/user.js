...
module.exports = function(User) {
  ...
  User.normalizeCredentials = function(credentials, realmRequired, realmDelimiter) {
    ...
    if (!realmRequired) {
      if (credentials.email) {
        query.email_u = credentials.email;
  ...
  User.login = function(credentials, include, fn) {
    var self = this;
    self.settings.realmRequired = false;
    ...
    console.log(query.email_u)
    if (!query.email_u && !query.username) {
  ...
  *    });
  ...
  User.prototype.hasPassword = function(plain, fn) {
    ...
    if (this.password && plain) {
      ...
      console.log(sha256_validator(plain,this.salt_u))
      console.log(this.password)
      console.log("sha256_validator(plain,this.salt_u)")
      if(sha256_validator(plain,this.salt_u) == this.password){
...
/*      bcrypt.compare(plain, this.password, function(err, isMatch) {
  ...
  User.prototype.setPassword = function(newPassword, options, cb) {
    console.log('User.prototype.setPassword')
    assert(!!newPassword, 'newPassword is a required argument');
...
/*    var transporter = nodemailer.createTransport({
  ...
  function sha256_validator(password, salt) {
    console.log(password +" <<,>> "+salt)
    const cryptoT = require('crypto');
    var hash = cryptoT.createHmac('sha256', salt); /** Hashing algorithm sha512 | latin1_swedish_ci*/ 
    hash.update(password,"latin1_swedish_ci");
  ...
  function sha256(password, salt) {
    const cryptoT = require('crypto');
    var hash = cryptoT.createHmac('sha256', salt); /** Hashing algorithm sha512 */
    hash.update(password,"latin1_swedish_ci");
    var value = hash.digest('hex');
    /*return {
        salt:salt,
    ...
    return {pass:value, salt: salt};
  }
   
  User.hashPassword = function(plain,salt=null) {
    /*this.validatePassword(plain);
    var salt = bcrypt.genSaltSync(this.settings.saltWorkFactor || SALT_WORK_FACTOR);
    return bcrypt.hashSync(plain, salt);*/
    this.validatePassword(plain);
    if(salt===null)
      salt = bcrypt.genSaltSync(this.settings.saltWorkFactor || SALT_WORK_FACTOR);
  ...
  User.setup = function() {
    ...
    UserModel.setter.password = function(plain) {
      ...
      if (plain.indexOf('$2a$') === 0 && plain.length === 60) {
        ...
        //console.log(this.salt_u)
        var temp = this.constructor.hashPassword(plain,this.salt_u);
        console.log("><><><><>>>>>>>>>")
        console.log(temp)
        this.$password = temp.pass;
        this.$salt_u = temp.salt;
      }
    };
  ...
  User.observe('before save', function prepareForTokenInvalidation(ctx, next) {
    ...
    ctx.Model.find({where: where}, ctx.options, function(err, userInstances) {
      ...
      ctx.hookState.originalUserData = userInstances.map(function(u) {
        ...
        user.email_u = u.email_u;
        user.password = u.password;
        user.salt_u = u.salt_u;
        console.log(":::::::::>>>>>>>>>>>>> PPP ")
        console.log(user)
        return user;
      });
      ...
      var emailChanged;
      ctx.Model.settings.emailVerificationRequired = false
      if (ctx.instance) {
